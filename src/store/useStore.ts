...
import { getDisplayLines, FontSize } from '../lib/scriptParser';

// Re-export FontSize so other files can import from either place
...
export const useStore = create<TeleprompterState>()(
  persist(
    (set, get) => ({
      ...
      goToStart: () => set({ currentLineIndex: 0, isPlaying: false }),

      goToEnd: () => {
        const { activeScriptId, scripts } = get();
        const script = scripts.find((sc) => sc.id === activeScriptId);
        const lines = getLinesForScript(script);
        set({ currentLineIndex: Math.max(0, lines.length - 1), isPlaying: false });
      },
      ...
      openSearch: () => set({ search: { isOpen: true, query: '', matches: [], currentMatchIndex: -1 } }),
      ...
      searchNext: () => {
        ...
        set({ search: { ...search, currentMatchIndex: next }, currentLineIndex: search.matches[next] });
      },
      ...
      searchPrev: () => {
        ...
        set({ search: { ...search, currentMatchIndex: prev }, currentLineIndex: search.matches[prev] });
      },
    }),
    {
